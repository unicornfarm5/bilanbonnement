...
def show_business_page(session_state_from_ui):
    ...
    #customers
    st.write("Hvor mange kunder har vi?")
    st.dataframe(get_all_customer_ids(session_state_from_ui))



#OBS det her er en funktion der kaldes i show_business_page
# all_rentals fetch
...
#customer id fanges her
def get_all_customer_ids(session_state_from_ui):
    TOKEN = session_state_from_ui.token
    headers = {"Authorization": f"Bearer {TOKEN}"} if TOKEN else {}
    ...
    if not session_state_from_ui.role: 
                    ...
                    try:
                        response = requests.get(f"{CUSTOMERSERVICE}/all_customer_id", headers=headers)
